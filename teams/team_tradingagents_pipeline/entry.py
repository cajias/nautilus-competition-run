"""Entry point for team_tradingagents_pipeline.

The harness invokes train(ctx) once per harness iteration. train(ctx) writes
fresh context, invokes Claude ONCE via run_claude, then loads the strategy.py
the Claude session produced.
"""
...
def train(ctx):
    ...
    result = run_claude(
        ...
        prompt=(
            ...
            f"Dispatch role agents via the Agent tool with the documented "
            f"`subagent_type` and `model` parameters. Produce "
            f"attempts/{iter_idx:03d}/strategy.py with classes "
            f"`TeamStrategy` and `TeamStrategyConfig`. Exit when done."
        ),
    ...
    spec = importlib.util.spec_from_file_location(
        f"team_strategy_{iter_idx:03d}", strategy_path
    )
