"""Seed the workspace catalog with real Binance BTCUSDT 5-minute klines.

Fetches a fully-closed 30-day window via Binance's public ``/api/v3/klines``
endpoint (no API key needed), builds a NautilusTrader BarType matching
``BTCUSDT.BINANCE-5-MINUTE-LAST-EXTERNAL``, and persists 8640 bars + the
``BTCUSDT.BINANCE`` ``CurrencyPair`` instrument into
``<workspace>/data/catalog/`` as a ParquetDataCatalog.

Window: 2026-04-01 00:00 UTC .. 2026-05-01 00:00 UTC (30 days, ending
7 days before today=2026-05-08 to avoid look-ahead). 8640 5-minute bars.
...
END_UTC = datetime(2026, 5, 1, 0, 0, tzinfo=timezone.utc)  # exclusive on the API; 30-day window

BINANCE_KLINES_URL = "https://api.binance.com/api/v3/klines"
PAGE_LIMIT = 1000          # Binance max per request
THROTTLE_SECONDS = 0.12    # gentle pause between pages
EXPECTED_BARS = 30 * 24 * 12  # 30 days × 288 5-min bars/day = 8640
