"""Entry point for team_kronos_forecast_committee.
...
def train(ctx):
    ...
    result = run_claude(
        ...
        prompt=(
            ...
            f"`TeamStrategy` and `TeamStrategyConfig` (Kronos REQUIRED in the Strategy; "
            f"transaction costs MUST be modeled). Exit when done."
        ),
    ...
    spec = importlib.util.spec_from_file_location(
        f"kronos_strategy_{iter_idx:03d}", strategy_path
    )
