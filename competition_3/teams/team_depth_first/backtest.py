"""Train-window backtest gate for team_adversarial — harness-faithful.
...
def _resolve_round_index() -> int:
    """Resolve the active round from _inbox/context.md (authoritative).

    The harness gates each round against its OWN train window; a hardcoded
    index silently measures every round against round 0. Fall back to 0 only
    if the context file is absent/unparseable.
    """
    ctx = TEAM_DIR / "_inbox" / "context.md"
    try:
        for line in ctx.read_text().splitlines():
            s = line.strip().lstrip("-").strip()
            if s.startswith("round_index:"):
...
def _load_strategy_module(strategy_path: Path):
    spec = importlib.util.spec_from_file_location("team_depth_first_candidate", strategy_path)
